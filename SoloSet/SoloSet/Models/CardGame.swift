...
struct CardGame {
	...
	init() {
		...
		for cardIndex in 0..<startCardsCount / 3 {
			let triangle = Triangle(shading: .open, color: .red, shapeCount: 3)
			let diamond = Diamond(shading: .open, color: .red, shapeCount: 3)
			let oval = Oval(shading: .open, color: .red, shapeCount: 2)
